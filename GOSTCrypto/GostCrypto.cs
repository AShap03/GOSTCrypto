...
public class GostCrypto
{
    ...
    {
        var encoded = Encode(
            Encoding.UTF8.GetBytes("12345678123456781234567812345678"),
            Encoding.UTF8.GetBytes("b14afb2db14afb2db14afb2db14afb2d"));
        Console.WriteLine(Encoding.UTF8.GetString(encoded));

        var decoded = Decode(encoded, Encoding.UTF8.GetBytes("b14afb2db14afb2db14afb2db14afb2d"));
        Console.WriteLine(Encoding.UTF8.GetString(decoded));
        Console.ReadKey();
    }

    protected static byte[][] SubstitutionBox =
    {
    ...
    protected static uint[] GenerateKeys(byte[] key)
    {
        if (key.Length != 32)
        {
    ...
    protected static uint Substitution(uint value)
    {
        uint output = 0;
    ...
    private static byte[] EncodeBlock(byte[] block, uint[] keys)
    {
    ...
    protected static byte[] Decode(byte[] data, byte[] key, bool isParallel = false)
    {
        var subkeys = GenerateKeys(key);
        var result = new byte[data.Length];
        var block = new byte[8];
    ...
    private static byte[] DecodeBlock(byte[] block, uint[] keys)
    {
        // separate on 2 blocks.
        uint N1 = BitConverter.ToUInt32(block, 0);
        uint N2 = BitConverter.ToUInt32(block, 4);
